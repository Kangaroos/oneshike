...
function find_dict($code, $is_add_all=false)
{
    if($is_add_all) {
        $dict = Dict::where("code",$code)->lists("para_name","para_code");
        $result = array(""=>"全部");
        foreach($dict as $key=>$val)
        {
            $result = array_add($result,$key,$val);
        }
        return $result;
    } else {
        return Dict::where("code",$code)->lists("para_name","para_code");
    }
}

function find_category($parent_code, $is_add_all=false)
{
    if($is_add_all) {
        $category = Category::where("parent_code",$parent_code)->lists("name","code");
        $result = array(""=>"全部");
        ...
        return Category::where("parent_code",$parent_code)->lists("name","code");
    }
}